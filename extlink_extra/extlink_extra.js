...
Drupal.behaviors.extlink_extra = {
  ...
  clickReaction: function(e) {
    ...
    Drupal.settings.extlink_extra.extlink_url_params['external_url'] = external_url;
    Drupal.settings.extlink_extra.extlink_url_params['back_url'] = back_url;
    ...
    if (Drupal.settings.extlink_extra.extlink_alert_type == 'colorbox') {
      if (Drupal.settings.extlink_extra.extlink_exceptions == 'yes'){ //If the user selected he wants a custom message
        //Create a colorbox
        $.colorbox({
          className: "extlink-extra-leaving-colorbox",
          height: "50%",
          href: "/externallinks.pfizer/now-leaving?js=1 .extlink-extra-leaving",
          initialHeight: "50%",
          initialWidth: "50%",
          onComplete: function (){
            for (var i = 0; i < Drupal.settings.extlink_extra.extlink_exceptions_list.length; i++){  //This for will check the array of exceptions so I can check if the current page have a certain selector
              if ($(document).has(Drupal.settings.extlink_extra.extlink_exceptions_list[i].title).length > 0){  //If there is one or more exceptions in the list.
                if ($(Drupal.settings.extlink_extra.extlink_exceptions_list[i].title).has(e.currentTarget).length > 0){   //If the current page have this selector.
                  $('.extlink-extra-leaving').html(Drupal.settings.extlink_extra.extlink_exceptions_list[i].text + '<div style="vertical-align:bottom; color:red; height:100%"><div style="width:50%; float:left;"><button value="Back" style="float:right; margin-right: 10px;" onclick="jQuery.colorbox.close();">Back</button></div><div style="width:50%; float:right;"><button value="Go" style="float:left; margin-left:10px;" onclick="window.location = \''+Drupal.settings.extlink_extra.extlink_url_params['external_url']+'\';">Go</button></div></div>');
                }
              }
            }
          },
